...
export default function ScoreGauge({ score, size="md" }: Props) {
  ...
  return <div className="relative inline-flex items-center justify-center" style={{width:d,height:d}}>
    <svg width={d} height={d} className="-rotate-90"><circle cx={d/2} cy={d/2} r={r} fill="none" stroke="currentColor" strokeWidth={s} className="text-gray-200" /><circle cx={d/2} cy={d/2} r={r} fill="none" stroke="currentColor" strokeWidth={s} strokeDasharray={c} strokeDashoffset={o} strokeLinecap="round" className={`${cl} transition-all duration-700`} /></svg>
    <span className={`absolute font-bold ${f} ${cl}`}>{pct}%</span>
  </div>;
}
